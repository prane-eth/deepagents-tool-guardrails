...
ToolInputGuardrail: TypeAlias = Callable[[dict[str, Any], str], GuardrailResult]
ToolOutputGuardrail: TypeAlias = Callable[[dict[str, Any], str], GuardrailResult]


class ToolGuardrailsMiddleware(AgentMiddleware):
    """Apply tool input/output guardrail checks around tool execution.

    Each input guardrail is called with `(tool_call_data, agent_name)`.
    Each output guardrail is called with `(tool_output_data, agent_name)`.
    ...
    def _run_input_guardrails(self, tool_call_data: dict[str, Any]) -> bool:
        return all(self._resolve_sync_guardrail_result(guardrail(tool_call_data, self._agent_name)) for guardrail in self._tool_input_guardrails)

    async def _arun_input_guardrails(self, tool_call_data: dict[str, Any]) -> bool:
        for guardrail in self._tool_input_guardrails:
            if not await self._resolve_async_guardrail_result(guardrail(tool_call_data, self._agent_name)):
                return False
        return True

    def _run_output_guardrails(self, tool_output_data: dict[str, Any]) -> bool:
        return all(self._resolve_sync_guardrail_result(guardrail(tool_output_data, self._agent_name)) for guardrail in self._tool_output_guardrails)

    async def _arun_output_guardrails(self, tool_output_data: dict[str, Any]) -> bool:
        for guardrail in self._tool_output_guardrails:
            if not await self._resolve_async_guardrail_result(guardrail(tool_output_data, self._agent_name)):
                return False
        return True
    ...
    def _tool_output_payload(self, result: ToolMessage | Command[Any]) -> dict[str, Any]:
    ...
    def wrap_tool_call(
        ...
        if self._tool_input_guardrails and not self._run_input_guardrails(cast("dict[str, Any]", request.tool_call)):
            return self._blocked_message(request=request, phase="input")

        result = handler(request)

        if self._tool_output_guardrails and not self._run_output_guardrails(self._tool_output_payload(result)):
    ...
    async def awrap_tool_call(
        ...
        if self._tool_input_guardrails and not await self._arun_input_guardrails(cast("dict[str, Any]", request.tool_call)):
            return self._blocked_message(request=request, phase="input")

        result = await handler(request)

        if self._tool_output_guardrails and not await self._arun_output_guardrails(self._tool_output_payload(result)):
            return self._blocked_message(request=request, phase="output")
